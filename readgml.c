...
// Funcao que imprime os vertices de maior grau ao de menor

void imprime_por_grau(NETWORK *network){
    int i, j, maior=0, id[33], usado=0, iguais=0;

    id[0]=-1;
    while(usado<34){
        for(i=0; i<34; i++){
            if(maior<network->vertex[i].degree){
                for(j=0; j<=usado; j++){
                    if(network->vertex[i].id==id[j])
                        iguais=1;
                }
                if(iguais==0){
                    maior=network->vertex[i].degree;
                    id[usado]=network->vertex[i].id;
                }
                iguais=0;
            }
        }
        printf("Vertice id = %d, Grau = %d\n", id[usado], maior);
        usado++;
        maior=0;
    }
...
int main (){
	NETWORK *rede;
	rede = malloc (sizeof(NETWORK));
	FILE *arquivo;

	arquivo = fopen("karate.gml", "r");
	read_network(rede, arquivo);
	imprime_por_grau(rede);
	free_network(rede);
	fclose(arquivo);
